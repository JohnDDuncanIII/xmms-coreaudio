...
static void osx_write_audio(gpointer data, int length)
{
	...
	//printf("osx_write_audio(): length %d\n",length);
	...
	{
		/*
		  output_time_offset += (output_total * 1000) / output.bps;
		  output_total = 0;
		  close(fd);
		  fd = open(device_name,O_WRONLY);
		*/

		// reset audio device if necessary;

		osx_setup_format(new_format, new_frequency, new_channels);
	}

	if (effects_enabled() && ep && ep->mod_samples)
	{
		length = ep->mod_samples(&data, length,input.format.xmms,input.frequency,input.channels);
	}

	/*
	  wait for space on device
	...
	*/
	...
	{
		length = osx_convert_func(&data, length);
	}

	if (effect.frequency == output.frequency)
	{
		// write data to output device
		//output_total += write_all(fd, data, length);
	}
	else
	{
		//printf("ERROR!! effect frequency not the same as output frequency!!!\n");
		//output_total += osx_downsample(data, length,effect.frequency,output.frequency);
	}
}
